...
import { CreateEditIncomeAndExpenses } from './components/create-income-and-expenses';
...
export class Router {
  ...
  async openRoute() {
    ...
    this.layoutElement.innerHTML = await fetch(newRoute.template).then((response) =>
      response.text()
    );
    ...
    if (newRoute.content) {
      ...
      this.mainContentElement.innerHTML = await fetch(newRoute.content).then((response) =>
        response.text()
      );
    }
